...
{
    public class FloorMonthComplaintType : AbstractMonthlyFacilityCubeService
    {
        private Cubes.FloorMonthComplaintType _Cube;
        private IEnumerable<Facts.Complaint> _Facts;

        protected override void Init(DataDimensions changes)
        {
            _Cube = GetQueryable<Cubes.FloorMonthComplaintType>()
                .Where(x => x.Facility.Id == changes.Facility.Id)
                .FirstOrDefault();

            if (_Cube == null)
            {
                _Cube = new Cubes.FloorMonthComplaintType();
                _Cube.Entries = new List<Cubes.FloorMonthComplaintType.Entry>();
                ...
                foreach (var floor in changes.Floors)
                {
                    ...
                    var prevDataCount = _Facts
                        ...
                        (x.Month.MonthOfYear == priorMonth.MonthOfYear && x.Month.Year == priorMonth.Year)
                            && x.ComplaintType.Name == complaintType.Name && x.Floor.Id == floor.Id)
                            .Count();
                    ...
                    if (cube == null)
                    {
                        cube = new Cubes.FloorMonthComplaintType.Entry();
