...
{
    public class FacilityMonthIncidentInjury : AbstractMonthlyFacilityCubeService
    {
        private Cubes.FacilityMonthIncidentInjury _Cube;
        private IEnumerable<Facts.IncidentReport> _Facts;

        protected override void Init(DataDimensions changes)
        {

            _Cube = GetQueryable<Cubes.FacilityMonthIncidentInjury>()
                .Where(x => x.Facility.Id == changes.Facility.Id)
                .FirstOrDefault();

            if (_Cube == null)
            {
                _Cube = new Cubes.FacilityMonthIncidentInjury();
                _Cube.Entries = new List<Cubes.FacilityMonthIncidentInjury.Entry>();
            ...
            int priorPatientDays)
        {
            foreach (var incidentTypeGroup in changes.IncidentTypeGroups)
            {
                ...
                foreach (var injury in changes.IncidentInjuries)
                {
                    ...
                    var prevDataCount = _Facts
                        .Where(x =>
                            ...
                            &&
                                x.IncidentInjuries.Contains(injury)
