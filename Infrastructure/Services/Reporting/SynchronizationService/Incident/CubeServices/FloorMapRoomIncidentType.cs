...
{
    public class FloorMapRoomIncidentType : AbstractService
    {
        ...
        protected override void Run(DataDimensions changes)
        {
            ...
            {
                var cube = GetQueryable<Cubes.FloorMapRoomIncidentType>()
                ...
                if (cube == null)
                {
                    cube = new Cubes.FloorMapRoomIncidentType();

                }

                cube.FloorMap = floorMap;
                cube.RoomEntries = new List<Cubes.FloorMapRoomIncidentType.RoomEntry>();
                ...
                Save<Cubes.FloorMapRoomIncidentType>(cube);
            }
        ...
        private void ProcessRoom(Dimensions.Room room,
            ...
            Cubes.FloorMapRoomIncidentType cube,
            IEnumerable<Facts.IncidentReport> facts)
        {
            ...
            if (roomEntry == null)
            {
                roomEntry = new Cubes.FloorMapRoomIncidentType.RoomEntry();
                cube.RoomEntries.Add(roomEntry);
            }

            roomEntry.FloorMapRoom = floorMapRoom;
            roomEntry.EntityEntries = new List<Cubes.FloorMapRoomIncidentType.EntityEntry>();

            foreach (var fact in facts)
            {
                roomEntry.EntityEntries.Add(new Cubes.FloorMapRoomIncidentType.EntityEntry()
                {
                    Component = fact.Id,
                    Date = fact.OccurredOnDate.HasValue ? fact.OccurredOnDate.Value : fact.DiscoveredOnDate.Value ,
                    IncidentTypes = fact.IncidentTypes,
                    IncidentTypeGroups = fact.IncidentTypeGroups,
