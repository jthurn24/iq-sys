...
{
    public class FacilityMonthIncidentTypeGroup : AbstractMonthlyFacilityCubeService
    {

        private Cubes.FacilityMonthIncidentTypeGroup _Cube;
        private IEnumerable<Facts.IncidentReport> _Facts;

        protected override void Init(DataDimensions changes)
        {

            _Cube = GetQueryable<Cubes.FacilityMonthIncidentTypeGroup>()
                .Where(x => x.Facility.Id== changes.Facility.Id)
                .FirstOrDefault();

            if (_Cube == null)
            {
                _Cube = new Cubes.FacilityMonthIncidentTypeGroup();
                _Cube.Entries = new List<Cubes.FacilityMonthIncidentTypeGroup.Entry>();
            ...
            int priorPatientDays)
        {
            foreach (var incidentTypeGroup in changes.IncidentTypeGroups)
            {
                ...
                Cubes.FacilityMonthIncidentTypeGroup.Entry cube = _Cube.Entries
                ...
                if (cube == null)
                {
                    cube = new Cubes.FacilityMonthIncidentTypeGroup.Entry();
