...
{
    public class QuarterlyIncidentByLocationView
    {
        ...
        public void SetData(QuarterMonths quarter,
            IEnumerable<FacilityMonthIncidentLocation.Entry> totals)
        {
            ...
            QuarterlyStatTable<IncidentLocation>
                .LoadTable<IncidentLocation, FacilityMonthIncidentLocation.Entry>(
        ...
        private void LoadChart(PieChart chart,
            IEnumerable<FacilityMonthIncidentLocation.Entry> data)
        {
            var sections = data.Select(x => x.IncidentLocation).Distinct();
            int colorIndex = 0;

            foreach (var section in sections)
            {
                var total = data.Sum(x => x.Total);
                var matchCount = data.Where(x => x.IncidentLocation == section).Sum(x => x.Total);
