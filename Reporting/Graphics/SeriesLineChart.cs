...
namespace IQI.Intuition.Reporting.Graphics
{
    public class SeriesLineChart
    {
        ...
        public static MemoryStream GenerateImage(string data, string options, int? width, int? height)
        {
            ...
            if (optionList.ContainsKey("ColorOffset"))
            {
                colorCounter = colorCounter +  Convert.ToInt32(optionList["ColorOffset"]);
            }
        ...
        public string SerializeOptionsForRender()
        {
            ...
            {
                builder.Append(":");
                builder.Append("ColorOffset");
                builder.Append("|");
                builder.Append(this.ColorOffset);
            }
