...
{
    public class Stage3 : BaseReportingProvider
    {

        public Stage3(ICubeRepository c, IDimensionRepository d)
            : base(c, d) { }
        ...
        public override Models.QICast.Statistic GetResult(Guid facilityGuid)
        {
            ...
            var priorCube = CubeRepository.GetFacilityMonthWoundStage(facility.Id, priorMonth.Id, type.Id)
                .Where(x => x.Stage.Name == "Stage III")
                .FirstOrDefault();
            ...
            result.Description = string.Concat("Stage III <br> P:", Math.Round(priorRate, 3), "<br> C:", Math.Round(currentRate, 3));
