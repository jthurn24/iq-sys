...
{
    public class Unstageable : BaseReportingProvider
    {

        public Unstageable(ICubeRepository c, IDimensionRepository d)
            : base(c, d) { }
        ...
        public override Models.QICast.Statistic GetResult(Guid facilityGuid)
        {
            ...
            var priorCube = CubeRepository.GetFacilityMonthWoundStage(facility.Id, priorMonth.Id, type.Id)
                .Where(x => x.Stage.Name == "Unstageable")
                .FirstOrDefault();
            ...
            result.Description = string.Concat("Unstageable <br> P:", Math.Round(priorRate, 3), "<br> C:", Math.Round(currentRate, 3));
