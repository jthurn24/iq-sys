...
{
    public class Fall : BaseReportingProvider
    {

        public Fall(ICubeRepository c, IDimensionRepository d)
            : base(c, d) { }
        ...
        public override Models.QICast.Statistic GetResult(Guid facilityGuid)
        {
            ...
            var priorCube = CubeRepository.GetFacilityMonthIncidentTypeGroup(facility.Id, priorMonth.Id)
                .Where(x => x.IncidentTypeGroup.Name == "Fall")
                .FirstOrDefault();
            ...
            result.Description = string.Concat("Falls <br> P:", Math.Round(priorRate, 3), "<br> C:", Math.Round(currentRate, 3));
