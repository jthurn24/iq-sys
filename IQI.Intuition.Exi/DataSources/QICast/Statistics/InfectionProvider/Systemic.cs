...
{
    public class Systemic : BaseReportingProvider
    {

        public Systemic(ICubeRepository c, IDimensionRepository d)
            : base(c, d) { }
        ...
        public override Models.QICast.Statistic GetResult(Guid facilityGuid)
        {
            ...
            var priorCube = CubeRepository.GetFacilityMonthInfectionType(facility.Id, priorMonth.Id)
                .Where(x => x.InfectionType.ShortName == "Systemic")
                .FirstOrDefault();
            ...
            result.Description = string.Concat("Systemic <br> P:", Math.Round(priorRate, 3), "<br> C:", Math.Round(currentRate, 3));
