...
{
    public class Uti : BaseReportingProvider
    {

        public Uti(ICubeRepository c, IDimensionRepository d)
            : base(c, d) { }
        ...
        public override Models.QICast.Statistic GetResult(Guid facilityGuid)
        {
            ...
            var priorCube = CubeRepository.GetFacilityMonthInfectionType(facility.Id, priorMonth.Id)
                .Where(x => x.InfectionType.ShortName == "UTI")
                .FirstOrDefault();
            ...
            result.Description = string.Concat("UTI <br> P:", Math.Round(priorRate, 3), "<br> C:", Math.Round(currentRate, 3));
